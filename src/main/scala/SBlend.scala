...
//    println(bits)
    ...
    println(transformStructure(sdnaDecoded,"Scene"))
    println("----------------------")
  ...
  def printStructure(sdna: StructureDNA, typeName: String): Unit = {
    ...
    struct.fields foreach {
      f => println("Field("+sdna.names(f.fieldName)+":"+sdna.types(f.fieldType)+") length: "+sdna.lenghts(f.fieldType))
    }

    println("----------------------")
  }

  def transformStructure(sdna: StructureDNA, typeName: String): Seq[FileBlockField] = {
    if (typeName == "void"){
      return Seq()
    }
    val typeID = sdna.types.indexOf(typeName)
    val struct = sdna.structureTypes filter { _.name == typeID} headOption
